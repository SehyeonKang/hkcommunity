...
개인 웹 프로젝트를 통해서 웹사이트를 개발할 때의 전체적인 큰 틀과 구성, 동작 원리 및 흐름등을 익히기위해서 기본적인 게시판 프로젝트를 개발했습니다.
강의와 책으로 배운 지식들을 듣고 따라서 코딩하는 것으로 끝나지않고 이를 바탕으로 배운 것외에 기능을 추가하며 직접 웹사이트를 개발해보아야 공부했던 것들을 좀 더 확실하게 정리하며 이해할 수 있을 것이라고 생각합니다.

그리고 웹사이트 개발에 그치지않고 로컬 환경에서만 작동하는 것이 아닌 실제로 다른 사람들도 쉽게 볼 수 있도록 배포하는 경험도 필요하다고 생각해서 배포를 하는 동시에 커밋을 하면 배포가 되도록 자동화하고, 도메인 추가와 https 설정을 추가하는 것까지 진행했습니다.
